...
export const conversationsApi = api.injectEndpoints({
  endpoints: (builder) => ({
    ...
    updateConversation: builder.mutation<Conversation, { id: number; data: UpdateConversationRequest }>({
      query: ({ id, data }) => ({
    ...
    sendMessage: builder.mutation<Message, { conversationId: number; data: SendMessageRequest }>({
